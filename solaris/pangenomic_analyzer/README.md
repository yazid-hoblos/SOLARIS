...
# Step 2: Pathway analysis from HMM results
solaris pangenomic_analyzer pathways \
    --hmm-results hmm_analysis_results.csv \
    --target-ecs ec_numbers.txt \
    --plot \
    --similarity-analysis
```
...
You may use `pangenomic_analyzer/access_genomes.py` to access the genomes of a species of interest. If no species specified, the script will download a comprehensive set of cyanobacteria strains in genomes/ by default. 
...
3. Pathway Analysis (pathways command)
   ├── Read HMM analysis results
...
For complete examples and sample datasets, see the `tests/` directory.